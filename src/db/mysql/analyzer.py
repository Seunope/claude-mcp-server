"""
Core analyzer functionality for MySQL database analysis with LLMs.
"""
import os
import json
import pandas as pd
...
from .database import MySQLDBManager
from ...llm.openai_client import OpenAIClient
from ...visualization import create_visualization
load_dotenv()

class MySQLDBAnalyzer:
    ...
    def __init__(
        ...
        self.db_manager = MySQLDBManager(logger)
    ...
    def translate_to_sql(self, request: str) -> Optional[str]:
        ...
        Your task is to write a MySQL query that will answer this request.
        ...
        Important MySQL specifics to remember:
        - Use backticks (`) for table and column names instead of double quotes
        - For date formatting, use DATE_FORMAT() instead of TO_CHAR()
        - Use CONCAT() instead of || for string concatenation
        - For regular expressions, use REGEXP instead of ~ 
        - MySQL doesn't support LATERAL joins, use subqueries or JOIN with derived tables instead
        """
    ...
    def process_request(self, request: str) -> Dict[str, Any]:
        ...
        # Execute the query
        data = self.db_manager.execute_query_to_dataframe(sql_query)
        if data is None or data.empty:
